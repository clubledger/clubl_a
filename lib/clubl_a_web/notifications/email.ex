defmodule ClubLA.Email do
  ...
  An Swoosh email struct can be delivered by a Swoosh mailer (see mailer.ex & user_notifier.ex). Eg:
      ...
      ClubLA.Email.confirm_register_email(user.email, url)
      |> ClubLA.Mailer.deliver()
  """

  use Phoenix.Swoosh,
    view: ClubLAWeb.EmailView,
    layout: {ClubLAWeb.EmailView, "email_layout.html"}
  ...
  defp from_name do
    Application.get_env(:clubl_a, :mailer_default_from_name)
  end

  defp from_email do
    Application.get_env(:clubl_a, :mailer_default_from_email)
  end
  ...
  defp get_unsubscribe_url(user, email_kind) do
    ClubLA.Accounts.NotificationSubscriptions.unsubscribe_url(user, email_kind)
  end
end
