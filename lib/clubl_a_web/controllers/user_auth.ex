defmodule ClubLAWeb.UserAuth do
  @moduledoc """
  ...
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_clubl_a_web_user_remember_me"
  ...
  def log_in_user(conn, user, params \\ %{}) do
    ...
    ClubLA.Logs.log_async("sign_in", %{user: user})
    ClubLA.MailBluster.sync_user_async(user)
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      ClubLAWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end

    if conn.assigns[:current_user] do
      ClubLA.Logs.log_async("sign_out", %{user: conn.assigns.current_user})
    end
  ...
  defp signed_in_path(current_user), do: ClubLAWeb.Helpers.home_path(current_user)
