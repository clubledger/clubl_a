defmodule ClubLAWeb.Router do
  use ClubLAWeb, :router

  alias ClubLAWeb.Router.Helpers, as: Routes
  import ClubLAWeb.UserAuth

  pipeline :browser do
    plug :accepts, ["html"]
    plug :fetch_session
    plug :fetch_live_flash
    plug :put_root_layout, {ClubLAWeb.LayoutView, :root}
    plug :protect_from_forgery
    plug :put_secure_browser_headers
    plug :fetch_current_user
    plug ClubLAWeb.SetLocalePlug, gettext: ClubLAWeb.Gettext
  ...
  scope "/", ClubLAWeb do
    ...
    live_session :public,
      on_mount: [
        {ClubLAWeb.UserOnMountHooks, :maybe_assign_user},
        {ClubLAWeb.UserOnMountHooks, :assign_color_scheme}
      ] do
  ...
  scope "/", ClubLAWeb do
    ...
    live_session :require_authenticated_user_for_onboarding,
      on_mount: [
        {ClubLAWeb.UserOnMountHooks, :require_authenticated_user},
        {ClubLAWeb.UserOnMountHooks, :assign_color_scheme}
      ] do
  ...
  scope "/admin", ClubLAWeb do
    ...
    live_session :require_admin_user,
      on_mount: [
        {ClubLAWeb.UserOnMountHooks, :require_admin_user},
        {ClubLAWeb.UserOnMountHooks, :assign_color_scheme}
      ] do
  ...
  if Mix.env() in [:dev, :test] do
    ...
    scope "/dev" do
      ...
      live_dashboard "/dashboard", metrics: ClubLAWeb.Telemetry
      ...
      scope "/emails" do
        ...
        get "/", ClubLAWeb.EmailTestingController, :index
        get "/sent", ClubLAWeb.EmailTestingController, :sent
        get "/preview/:email_name", ClubLAWeb.EmailTestingController, :preview
        post "/send_test_email/:email_name", ClubLAWeb.EmailTestingController, :send_test_email
        get "/show/:email_name", ClubLAWeb.EmailTestingController, :show_html
      end
    end
  ...
  defp onboard_new_users(conn, _opts) do
    if conn.assigns[:current_user] && !conn.assigns.current_user.is_onboarded do
      conn
      |> redirect(to: Routes.live_path(conn, ClubLAWeb.UserOnboardingLive))
      |> halt()
    else
      conn
    end
  ...
  # For live view it sets it on the session (use {ClubLAWeb.UserOnMountHooks, :assign_color_scheme} as a hook - see https://hexdocs.pm/phoenix_live_view/security-model.html#mounting-considerations)
