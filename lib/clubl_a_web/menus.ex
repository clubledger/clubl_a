defmodule ClubLAWeb.Menus do
  ...
  def get_link(:settings, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ClubLAWeb.EditProfileLive),
      icon: :cog
    }
  end
  ...
  def get_link(:edit_email, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ClubLAWeb.EditEmailLive),
      icon: :at_symbol
    }
  end
  ...
  def get_link(:edit_notifications, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ClubLAWeb.EditNotificationsLive),
      icon: :bell
    }
  end
  ...
  def get_link(:edit_password, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ClubLAWeb.EditPasswordLive),
      icon: :key
    }
  end
  ...
  def get_link(:dashboard, _current_user) do
    %{
      ...
      path: Routes.live_path(Endpoint, ClubLAWeb.DashboardLive),
      icon: :template
    }
  end
  ...
  def get_link(:dev_email_templates = name, _current_user) do
    if Application.get_env(:clubl_a, :env) == :dev do
      %{
        name: name,
  ...
  def get_link(:dev_page_builder = name, _current_user) do
    if Application.get_env(:clubl_a, :env) == :dev do
      %{
        name: name,
        label: "Page builder",
        path: Routes.live_path(Endpoint, ClubLAWeb.PageBuilderLive),
        icon: :document_add
      }
    else
      nil
