...
config :clubl_a, ClubLA.Repo,
  username: "postgres",
  password: "postgres",
  database: "clubl_a_test#{System.get_env("MIX_TEST_PARTITION")}",
  hostname: "localhost",
...
config :clubl_a, ClubLA.Mailer, adapter: Swoosh.Adapters.Test
...
config :clubl_a, :env, :test

# Wallaby related settings:
config :wallaby, otp_app: :clubl_a
config :clubl_a, :sandbox, Ecto.Adapters.SQL.Sandbox

# Oban - Disable plugins, enqueueing scheduled jobs and job dispatching altogether when testing
config :clubl_a, Oban, queues: false, plugins: false
